...
document.addEventListener('contextmenu',e=>{state.contextPoint={x:e.clientX,y:e.clientY}},true);document.addEventListener('click',e=>{if(!e.target.closest('#contextMenu'))$('contextMenu').style.display='none';if(!e.target.closest('#columnMenu,#toggleColumns'))closeColumnMenu()});window.addEventListener('blur',()=>{$('contextMenu').style.display='none';closeColumnMenu()});window.addEventListener('resize',()=>{if($('columnMenu').classList.contains('open'))positionColumnMenu()});document.addEventListener('keydown',e=>{if(e.key==='Escape'){$('contextMenu').style.display='none';closeColumnMenu()}});
