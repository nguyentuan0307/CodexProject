...
export class GitLogViewProvider implements vscode.WebviewViewProvider, vscode.Disposable {
  ...
  private async handle(message: WebviewMessage): Promise<void> {
    ...
    try {
      ...
      this.post(recovery ? { type: 'recovery', recovery, operation: 'CHERRY-PICKING' } : { type: 'error', message: text, scope: message.type });
    }
  }
...
function contextActions(kind?: string, current = false): GitContextAction[] {
  if (kind === 'local') return [
    ...(current ? [
      ...
      contextAction('checkoutUpdate', undefined, 'more'), contextAction('updateBranchFromOrigin', 'Update from Origin', 'more'),
      contextAction('createBranch', 'Create Branch from Here…', 'more'), contextAction('renameBranch', 'Rename Branch…', 'more'),
      contextAction('workingDiff', 'Compare with Working Tree', 'more'), contextAction('checkoutRebase', undefined, 'more'),
      contextAction('worktreeAdd', 'Create Worktree…', 'more'), contextAction('copy', 'Copy Branch Name', 'more'),
      contextAction('deleteBranch', 'Delete Branch', 'more'), contextAction('forceDeleteBranch', 'Force Delete Branch', 'danger')
    ])
  ];
...
function renderColumnMenu(){const visibleCount=visibleColumns.size,actions=[['update','Update Current Branch'],['createBranch','Create Branch…'],['stash','Stash Changes…'],['pushOptions','Push Options…']];$('columnMenu').innerHTML=actions.map(item=>'<button data-menu-action="'+item[0]+'">'+item[1]+'</button>').join('')+'<div class="menu-separator"></div><small>Commit columns · Keep at least one column visible.</small>'+COMMIT_COLUMNS.map(c=>'<label><input type="checkbox" data-column-toggle="'+c.id+'" '+(visibleColumns.has(c.id)?'checked':'')+' '+(visibleColumns.has(c.id)&&visibleCount===1?'disabled':'')+'> '+c.label+'</label>').join('')}
...
function showRecoveryToast(message){const recovery=message.recovery,toast=$('toast');$('toastTitle').textContent=recovery.title;$('toastMessage').textContent=recovery.kind==='pushRejected'?'Update the branch, review incoming commits, then push again.':'Choose how to continue the current operation.';$('toastDetailText').textContent=recovery.detail;$('toastDetails').style.display='block';$('toastDetails').open=false;$('toastActions').innerHTML='';for(const item of recovery.actions){const button=document.createElement('button');button.textContent=item.label;button.onclick=()=>{hideToast();send('mutate',{action:item.action,operation:message.operation})};$('toastActions').appendChild(button)}toast.classList.add('recovery');toast.classList.remove('success');toast.style.display='block'}
...
$('files').addEventListener('click',e=>{const file=e.target.closest('.file');if(!file)return;state.selectedFilePath=file.dataset.path;renderFiles(state.visibleFiles,state.visibleFilesWorking)});$('files').onkeydown=e=>{const files=state.visibleFiles;if(!files.length)return;let index=Math.max(0,files.findIndex(file=>file.path===state.selectedFilePath));if(e.key==='ArrowDown'||e.key==='ArrowUp'){e.preventDefault();index=Math.max(0,Math.min(files.length-1,index+(e.key==='ArrowDown'?1:-1)));state.selectedFilePath=files[index].path;renderFiles(files,state.visibleFilesWorking);$('files').querySelector('[data-path="'+CSS.escape(state.selectedFilePath)+'"]')?.scrollIntoView({block:'nearest'})}else if(e.key==='Enter'){e.preventDefault();const file=files[index];if(!file)return;state.visibleFilesWorking?send('workingDiff',{path:file.path}):state.detail&&send('diff',{hash:state.detail.hash,path:file.path,parent:Number($('parentMode').value)||1})}};
...
function renderContextMenuGroup(group='primary'){const menu=$('contextMenu'),actions=state.contextActions||[],primary=actions.filter(item=>item.group==='primary'),more=actions.filter(item=>item.group==='more'),danger=actions.filter(item=>item.group==='danger'),button=item=>'<button class="'+(item.group==='danger'?'danger':'')+'" role="menuitem" data-context-action="'+esc(item.action)+'">'+esc(item.label)+'</button>';if(group==='more'){menu.innerHTML='<button class="context-back" role="menuitem" data-context-group="primary">‹ Back</button><div class="context-separator"></div>'+more.map(button).join('')}else{menu.innerHTML=primary.map(button).join('')+(more.length?'<div class="context-separator"></div><button class="context-more" role="menuitem" data-context-group="more">More Actions</button>':'')+(danger.length?'<div class="context-separator"></div>'+danger.map(button).join(''):'')}menu.querySelector('button')?.focus()}
function showInlineContextMenu(message){const menu=$('contextMenu'),context=message.context;if(state.busy||context.requestId!==state.contextRequestId)return;state.contextPayload=context;state.contextActions=message.actions;if(!message.actions.length)return;renderContextMenuGroup();menu.style.display='block';menu.style.left='0';menu.style.top='0';const point={x:context.x??state.contextPoint?.x??0,y:context.y??state.contextPoint?.y??0},rect=menu.getBoundingClientRect();menu.style.left=Math.max(4,Math.min(point.x,window.innerWidth-rect.width-4))+'px';menu.style.top=Math.max(4,Math.min(point.y,window.innerHeight-rect.height-4))+'px'}
...
$('contextMenu').onclick=e=>{const group=e.target.closest('[data-context-group]');if(group){renderContextMenuGroup(group.dataset.contextGroup);return}const button=e.target.closest('[data-context-action]');if(!button||state.busy)return;button.disabled=true;const {type,...context}=state.contextPayload;send('contextAction',{...context,action:button.dataset.contextAction});$('contextMenu').style.display='none'};$('contextMenu').onkeydown=e=>{const buttons=[...$('contextMenu').querySelectorAll('button:not([disabled])')],index=buttons.indexOf(document.activeElement);if(e.key==='ArrowDown'||e.key==='ArrowUp'){e.preventDefault();buttons[(index+(e.key==='ArrowDown'?1:-1)+buttons.length)%buttons.length]?.focus()}else if(e.key==='Home'){e.preventDefault();buttons[0]?.focus()}else if(e.key==='End'){e.preventDefault();buttons.at(-1)?.focus()}else if(e.key==='ArrowRight'&&document.activeElement?.dataset.contextGroup==='more'){e.preventDefault();renderContextMenuGroup('more')}else if(e.key==='ArrowLeft'&&state.contextActions?.some(item=>item.group==='more')){e.preventDefault();renderContextMenuGroup('primary')}};window.addEventListener('message',e=>{const m=e.data;if(m.type==='contextMenu')showInlineContextMenu(m);if(m.type==='busy'&&m.busy)$('contextMenu').style.display='none';if(m.type==='state'&&m.repository?.operation)setTimeout(()=>{const conflicts=m.uncommitted.filter(file=>file.conflict);$('operation').textContent=m.repository.operation+' · '+conflicts.length+' unresolved';document.querySelector('[data-conflict="continue"]').disabled=conflicts.length>0;document.querySelector('[data-conflict="skip"]').style.display=/REBAS|CHERRY/.test(m.repository.operation)?'inline-block':'none'},0)},true);
