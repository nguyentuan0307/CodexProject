...
function lockIcon(){return'<span class="protected-icon" title="Protected branch"><svg class="ui-icon" viewBox="0 0 24 24"><rect x="5" y="10" width="14" height="10" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/></svg></span>'}function protectedBranch(name){const special='\\.^$+?()[]{}|';return(state.protectedBranches||[]).some(pattern=>{const escaped=[...String(pattern)].map(c=>c==='*'?'.*':special.includes(c)?'\\\\'+c:c).join('');return new RegExp('^'+escaped+'$').test(name)})}function refItem(x,label=x.name,depth=0){return '<div class="item '+(x.current?'active ':'')+(state.selectedRef===x.name?'viewing':'')+'" style="padding-left:'+(8+depth*14)+'px" data-kind="'+x.kind+'" data-hash="'+x.hash+'" data-ref="'+esc(x.name)+'"><button style="border:0;background:transparent;height:auto;padding:0 4px 0 0" data-star="'+esc(x.name)+'" title="Favorite">'+(state.favorites.has(x.name)?'★':'☆')+'</button><span class="ref-icon" aria-hidden="true"><span class="ref-shape"></span></span>'+esc(label)+(protectedBranch(x.name)?lockIcon():'')+'<span class="badge">'+(x.ahead?'↑ '+x.ahead:'')+(x.ahead&&x.behind?'  ':'')+(x.behind?'↓ '+x.behind:'')+'</span></div>'}
