...
function send(type,data={}){vscode.postMessage({type,...data})}function date(ts){return new Date(ts*1000).toLocaleString()}function repositoryStatus(r){return r?r.head+'  ↑'+r.ahead+' ↓'+r.behind+'  '+r.changedCount+' changed':''}function actionLabel(action){return String(action||'Git operation').replace(/([A-Z])/g,' $1').toLowerCase()}
