...
<script nonce="${nonce}">
const vscode=acquireVsCodeApi(), ROW=28, PAGE=200, overscan=12;let state={commits:[],total:0,loading:new Set(),selected:-1,selectedHashes:new Set(),detail:null,uncommitted:[],visibleFiles:[],visibleFilesWorking:false,fileFolders:new Set(),fileMode:localStorage.getItem('gitLog.fileMode')||'tree',fileCollapsed:new Set(JSON.parse(localStorage.getItem('gitLog.fileCollapsed')||'[]')),favorites:new Set(JSON.parse(localStorage.getItem('gitLog.favorites')||'[]')),collapsed:new Set(JSON.parse(localStorage.getItem('gitLog.collapsed')||'[]'))};
...
$('contextMenu').onclick=e=>{const button=e.target.closest('[data-context-action]');if(!button)return;send('contextAction',{...state.contextPayload,action:button.dataset.contextAction});$('contextMenu').style.display='none'};window.addEventListener('message',e=>{if(e.data.type==='contextMenu')showInlineContextMenu(e.data)},true);
