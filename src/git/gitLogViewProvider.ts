...
function refItem(x,label=x.name,depth=0){const icon=x.current?'●':x.kind==='remote'?'⇄':x.kind==='tag'?'◆':'⑂';return '<div class="item '+(x.current?'active':'')+'" style="padding-left:'+(8+depth*14)+'px" data-kind="'+x.kind+'" data-hash="'+x.hash+'" data-ref="'+esc(x.name)+'"><button style="border:0;background:transparent;height:auto;padding:0 4px 0 0" data-star="'+esc(x.name)+'" title="Favorite">'+(state.favorites.has(x.name)?'★':'☆')+'</button><span class="ref-icon">'+icon+'</span>'+esc(label)+'<span class="badge">'+(x.ahead?'↑ '+x.ahead:'')+(x.ahead&&x.behind?' · ':'')+(x.behind?'↓ '+x.behind:'')+'</span></div>'}
